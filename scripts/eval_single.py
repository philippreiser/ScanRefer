...
SCANREFER_TRAIN = json.load(open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_train_over.json")))
SCANREFER_VAL = json.load(open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_val_over.json")))
...
def get_model(args, config):
    ...
    print(path)
    model.load_state_dict(torch.jit.load(path), strict=False)
    model.eval()
